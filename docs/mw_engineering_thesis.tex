...
In order to allow the program user a degree of control over the generation process, a rudimentary user interface has been prepared. \Cref{fig:uiwindows} shows the model of its classes. The class named \textit{UserInterface} acts as an update and rendering controller for each of the windows represented by the other classes in the model, which can call functions bound to UI controls they contain and show a visual representation of a \textit{Board} or \textit{Map} by using the data encapsulated by \textit{SimpleTexture2D}.
