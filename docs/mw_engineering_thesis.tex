...
\td{Shape grammars used to construct models of mazes, dungeons. Substitution: shapes from chosen starting set are transformed into more complex shapes which contain more shapes, process then repeated }
...
Another experiment, shown in \cref{fig:ex1}, uses rules where the radius of Moore neighbourhood is increased $(r = 2)$ and walls are placed whenever there are more than 12 adjacent walls to the central cell. In this case, islands produced by transformation are not completely solid, which may be a useful feature to map designers.

\begin{figure}[h]
	\centering
	\includegraphics[height=5cm]{images/r2ex1}
	\caption{Left: one of generated tiles with transformation rule: if n > 12, set cell to wall state. Right: construction of experimental map on 5x5 grid. Source: own work}
	\label{fig:r2ex1}
\end{figure}

In closing: images in this section illustrate the utility of employing CA for generating map assets, however finding the right rules and parameters may require additional trials.
...
The solution in its current form is sufficient for further experimentation needs. However, it may be worth noting what kind of improvements could be implemented, in effect adapting it to an environment where generated maps will be used as game assets.
