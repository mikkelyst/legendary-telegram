...
The aim of this thesis was to provide a software solution which uses cellular automata for constructing maps. Although the goal of creating a program for map construction has been achieved, a question stands: are cellular automata useful for generation of interesting, surprising and unique maps, suitable for use in production of games?

Observing the results presented in figures produced by implemented generator reveal that cellular automata can be used for generation of desirable forms, although finding a set of rules which produce tiles that contain surprising constructs may be challenging. Tiles generated by simple rules from random noise are visually similar to each other, despite having unique layouts.

\td{designer control at cost of tedious clicking}

\td{The procedure for constructing maps involves a degree of user interaction, which provides some control over map construction process...}

\td{Co do conclusion to prosze zebrać wnioski, nie przemyślenia, z całego tego projektu i opisać wady i zalety Pana rozwiązania. Czemu jest inne niz wszystkie co wprowadza nowego a co starego naprawia.}
...
\subsubsection{Cost of tile generation}
...
\td{future: more cell types, user defined cell types}
\td{future: non-discrete cell states or complex cell types }
