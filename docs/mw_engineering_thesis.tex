...
Such model allows to simulate some basic cellular automata based on rules which require a sum of values in cells adjacent to the selected cell to decide its future state.
 
 \td{add code listings}
 
The next section presents how rules of CA simulation are encapsulated and applied in the implemented map generator.
...
\td{board before into board after with search for match in rule set. one rule is a function that maps a single set of cell states to one resulting cell state}


\subsubsection{Operations}
\td{applying CA rule to board}
