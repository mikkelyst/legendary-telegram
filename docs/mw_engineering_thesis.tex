...
The following sections shortly discuss the possibility of applying approaches found in described surveys to generation of planar maps with irregular shapes (which is the goal of this thesis).
...
\begin{itemize}
	...
	\item state of adjacent cells,
	\item positions of neighbours with state $s$
	\item \td{what else CA rules may measure?}
\end{itemize}
...
Since this thesis is focused on creation of maps through procedural processes, it is worth noting why cellular automata have been chosen as the method to develop a solution. The preceding sections have shown that CA are a sufficient approach to map generation and are simpler to implement in comparison to other solutions. Additionally, CA have the following characteristics:

\begin{itemize}
	\item similarity to some image processing algorithms (e.g. erosion, dilation)
	\item are able to model evolution of dynamical systems (e.g. fluids) - which may be useful to create maps of terrain similar to natural landmass forms
	\item \td{more?}
\end{itemize}

Although generative grammars are a viable alternative, they rely on defined sets of symbols to generate content. Compared to CA, grammar approach seems to require more effort to create irregular, chaotic forms in shapes of natural terrain.

\td{are grammars optimal for generating irregular shapes?}
...
To describe the developed solution, this chapter consists of five sections: definition of required features, design of simple CA simulation and map generator models, followed by implementing them in C++ programming language and description of performed experiments.
...
We can also assign a number to each cell \td{why?} as shown in table \cref{tab:cellneighbors}.
