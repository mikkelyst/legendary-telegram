...
Historically, maps have been used by the human race since ancient times. Starting with cave paintings and representations of stars on the sky, our kind had the need for capturing an abstract model of a territory, terrain shape, location of useful resources or some other aspect of surrounding environment in a useful way.

\td{expand}
\underline{(more information)} in Bagrow's History of Maps \autocite{bagrow2017history}.

The need for navigation in the world has been a driving force behind the evolution of cartography. Although early maps had the form of drawings, there are other possibilities of representing the abstract model which a map aims to represent.
\td{describe map classification}

\td{examples of 2d maps, 3d maps, pure data maps}
...
It is not clear what kind of game was the first one in history to use a map to represent the game world, however two notable examples may easily come into mind. Chess, a board tactical war-game developed before 6th century AD, uses oa 8 by 8 black-white board as the map of its world.
...
\td{chinese Go, with map variants} 
...
\begin{figure}[h]
	...
	\label{fig:map}
\end{figure}
\todo{find out what game its from}
